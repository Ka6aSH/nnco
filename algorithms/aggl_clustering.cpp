...
AgglClustering::AgglClustering(std::vector<Point *> *points,
                               ...
                               std::pair<double (*)(Point *, Point *), double (*)(double, double, int)> distance_func,
                               int cluster_number) :
...
void AgglClustering::Init() {
    alg->Init(&points, metric.first, metric.second);
}
...
void AgglClustering::RunClustering() {
    ...
    for (int i = 0; i < points.size(); ++i) {
        ...
        heap.push({metric.first(points[i], nn), points[i], nn});
    }
    while (clusters.size() > cluster_number) {
        ...
        if (!alg->Contains(ct.first)) {
            ...
            heap.push({metric.first(ct.first, nn), ct.first, nn});
        } else {
            // Remove old clusters
            ...
            if (nearest != nullptr)
                heap.push({metric.first(merged->get_center(), nearest), merged->get_center(), nearest});
        }
    }
